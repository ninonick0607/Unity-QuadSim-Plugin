...
namespace Yaml.Drone
{
    public class YamlParser
    {
        ...
        )
        {
            ...
            try
            {
                yamlText = File.ReadAllText(fullPath);
            }
            catch (Exception e)
            {
                Debug.LogError($"[Drone.YamlParser] Failed to read file: {fullPath}\n{e}");
                return false;
            }

            return LoadDroneConfigFromString(
                yamlText,
                fullPath,
                out outModel,
                out outPhys,
        ...
        public static bool LoadDroneConfigFromString(
            string yamlText,
            string sourceLabel,
            out ModelType outModel,
            out DroneParameters outPhys,
        ...
        )
        {
            ...
            {
                Debug.LogError($"[Drone.YamlParser] YAML text is empty: {sourceLabel}");
                return false;
            }
            ...
            try
            {
                using (var reader = new StringReader(yamlText))
                {
                    ...
                    {
                        Debug.LogError($"[Drone.YamlParser] YAML has no documents: {sourceLabel}");
                        return false;
                    }

                    if (!(yaml.Documents[0].RootNode is YamlMappingNode mapping))
                    {
                        Debug.LogError($"[Drone.YamlParser] YAML root is not a mapping node: {sourceLabel}");
                        return false;
                    }
            ...
            {
                Debug.LogError($"[Drone.YamlParser] YAML load exception in {sourceLabel}: {e.Message}");
                return false;
            }

            return ParseDroneConfigNode(
                rootNode,
                sourceLabel,
                out outModel,
                out outPhys,
        ...
        private static bool ParseDroneConfigNode(
            YamlMappingNode rootNode,
            string sourceLabel,
            out ModelType outModel,
            out DroneParameters outPhys,
        ...
        )
        {
            ...
            try
            {
                ...
                {
                    // Mesh path is kept in YAML for future model catalog support.
                    // Runtime prefab/model loading is handled elsewhere for now.
                }
            ...
            {
                Debug.LogError($"[Drone.YamlParser] Parsing logic error in {sourceLabel}: {e.Message}\n{e.StackTrace}");
                return false;
            }
        }
    }
